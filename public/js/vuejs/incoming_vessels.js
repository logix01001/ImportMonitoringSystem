...
new Vue({
    ...
    methods: {
        searchDate(date = null,reference = null,as_of_now = false){
            var self = this
            self.as_of_now = as_of_now
            $.blockUI({ message: '<h3> Processing ...</h3>' }); 

            axios.post(`http://${window.location.host}/${myBaseName}api/bill_of_lading/incoming_vessels`,
            {
                ...
                self.list_distinct = res.data[0]
                self.list_bl_volume = res.data[1]
        ...
        showDetails(distinct=null,factory=null,category=null,pod=null,pod_all = false,factory_all = false){
            var self = this
            ...
            axios.post(`http://${window.location.host}/${myBaseName}api/bill_of_lading/incoming_vessels_details`,
            {
                ...
                $('.myModal5').modal('show');
                if( self.distinct){
                    self.details_distinct = res.data
                }else{
