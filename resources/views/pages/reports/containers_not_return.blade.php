        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    ...
                    <h5>Containers not yet return</h5>
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    <tr v-for="(ld,index) in list_bl_volume"  v-if="index != list_bl_volume.length - 1 ">
                                        <td>@{{ ld.name }}</td> 
                                        <td  class="cursor" @click="showDetails(ld.name,false)">@{{ ld.volume }}</td>
                                        <td >
                                            <ul class="list-group clear-list m-t">
                                                <li class="list-group-item" :class="(index == 0 ) ? 'fist-item' : ''" v-for="(ctype,index) in sizeCountContainer(ld.size)" v-if="ctype.container_type != ''">
                                                        @{{ctype.count + ' x ' + ctype.container_type }}
                                                </li>
                                            </ul>
                                        </td>
                                    </tr>
                                    <tr style="font-weight: bold; background-color:yellow;">
                                            <td>Total</td> 
                                            <td  class="cursor" @click="showDetails('false',true)"> @{{ list_bl_volume[list_bl_volume.length - 1]['volume'] }}</td>
                                            <td >
                                                <ul class="list-group clear-list m-t">
                                                    <li class="list-group-item"  v-for="(ctype,index) in sizeCountContainer(list_bl_volume[list_bl_volume.length - 1]['size'])" v-if="ctype.container_type != ''">
                                                            @{{ctype.count + ' x ' + ctype.container_type }}
                                                    </li>
                                                </ul>
                                            </td>
...
@section('vuejsscript')
    <script src="{{asset('/js/vuejs/containers_not_return.js')}}"></script>
@endsection
